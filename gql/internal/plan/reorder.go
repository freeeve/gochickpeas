...
// adds a subquery's pattern variables too -- the caller filters against
...
func collectAllVars(e ast.Expr, out map[string]bool) {
	...
	case *ast.Cost:
		...
		out[n.To] = true
	case *ast.Unary:
		collectAllVars(n.Expr, out)
	case *ast.IsNull:
		collectAllVars(n.Expr, out)
	case *ast.Binary:
		collectAllVars(n.LHS, out)
		collectAllVars(n.RHS, out)
	case *ast.Func:
	...
	case *ast.In:
		collectAllVars(n.Expr, out)
		collectAllVars(n.List, out)
	case *ast.Case:
		if n.Operand != nil {
			collectAllVars(n.Operand, out)
		}
		for _, w := range n.Whens {
			collectAllVars(w.Cond, out)
			collectAllVars(w.Result, out)
		}
		if n.Else != nil {
			collectAllVars(n.Else, out)
		}
	case *ast.Exists:
		collectPatternVars(n.Pattern, out)
		if n.Where != nil {
			collectAllVars(n.Where, out)
		}
	case *ast.CountSub:
		collectPatternVars(n.Pattern, out)
		if n.Where != nil {
	...
	case *ast.PatternComp:
		...
		collectAllVars(n.Proj, out)
	case *ast.ListPred:
		out[n.Var] = true
		collectAllVars(n.List, out)
		collectAllVars(n.Pred, out)
	case *ast.Reduce:
		out[n.Acc] = true
		out[n.Var] = true
		collectAllVars(n.Init, out)
		collectAllVars(n.List, out)
		collectAllVars(n.Body, out)
	case *ast.ListComp:
		out[n.Var] = true
		collectAllVars(n.List, out)
		if n.Filter != nil {
			collectAllVars(n.Filter, out)
		}
		if n.Map != nil {
			collectAllVars(n.Map, out)
		}
	case *ast.Index:
		collectAllVars(n.Base, out)
		collectAllVars(n.Idx, out)
	case *ast.Slice:
		collectAllVars(n.Base, out)
		if n.From != nil {
			collectAllVars(n.From, out)
		}
		if n.To != nil {
			collectAllVars(n.To, out)
		}
	case *ast.PropOf:
		collectAllVars(n.Base, out)
	case *ast.MapLit:
		for _, f := range n.Fields {
			collectAllVars(f.Val, out)
		}
	}
}

func collectPatternVars(p *ast.Pattern, out map[string]bool) {
