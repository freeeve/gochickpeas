...
func propagateRows(t *testing.T, rows *gql.Rows) map[string][2]int64 {
	t.Helper()
	names := []string{"s", "a", "b", "c"}
...
func TestCallPropagateCorrelated(t *testing.T) {
	...
	rows := gql.RunBoth(t, g, `MATCH (root:Acct {name: 's'})-[d:flow]->(x)
...
func TestCallCorrelatedBfsAndRuntimeMismatch(t *testing.T) {
	...
	rows = gql.RunBoth(t, g, `MATCH (p:Acct {name: 's'}) LET bad = p.name
...
func TestCallCdlpAndLcc(t *testing.T) {
	...
	rows := gql.RunBoth(t, g, "CALL algo.cdlp(false, 10) YIELD node, value RETURN node, value")
	for r := range rows.All() {
		nv, _ := r.Get("node")
		id, _ := nv.AsNode()
		vv, _ := r.Get("value")
	...
	wantL := g.LCC(false)
	rows = gql.RunBoth(t, g, "CALL algo.lcc(false) YIELD node, value RETURN node, value")
	for r := range rows.All() {
		nv, _ := r.Get("node")
		id, _ := nv.AsNode()
		vv, _ := r.Get("value")
