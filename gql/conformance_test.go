...
func conformanceCorpus() []conformanceProbe {
	return []conformanceProbe{
		...
		{"A composition", "match-mode-repeatable", "MATCH REPEATABLE ELEMENTS (a)-[:KNOWS]->(b) RETURN count(*) AS n", -1, "", "expected '(' starting a node pattern, found \"REPEATABLE\"", nil},
		{"A composition", "match-mode-different", "MATCH DIFFERENT EDGES (a)-[:KNOWS]->(b) RETURN count(*) AS n", -1, "", "expected '(' starting a node pattern, found \"DIFFERENT\"", nil},
		...
		{"B patterns", "node-inline-where", "MATCH (p:Person WHERE p.age > 26) RETURN count(*) AS n", -1, "", "expected ')' closing a node pattern, found \"WHERE\"", nil},
		...
		{"B patterns", "label-wildcard-pct", "MATCH (x:%) RETURN count(*) AS n", -1, "", "unexpected character \"%\"", nil},
		...
		{"B patterns", "rel-inline-where", "MATCH (a)-[r:KNOWS WHERE a.age > 0]->(b) RETURN count(*) AS n", -1, "", "expected ']' closing a relationship pattern, found \"WHERE\"", nil},
		...
		{"B patterns", "quant-question", "MATCH (a {name:'Alice'})-[:KNOWS]->?(b) RETURN count(*) AS n", -1, "", "unexpected character \"?\"", nil},
		...
		{"C expressions", "modulo-pct", "RETURN 7 % 2 AS x", -1, "", "unexpected character \"%\"", nil},
		...
		{"C expressions", "nullif", "RETURN NULLIF(1, 1) AS x", -1, "", "unknown function `NULLIF`", nil},
		...
		{"C expressions", "string-single-quote", "RETURN 'it''s' AS x", -1, "", "unexpected trailing input \"s\"", nil},
		{"C expressions", "string-backslash-escape", "RETURN 'a\\nb' AS x", 1, "", "", nil},
		{"C expressions", "sci-float", "RETURN 1.5e2 AS x", -1, "", "unexpected trailing input \"e2\"", nil},
		...
		{"D functions", "stddev", "MATCH (p:Person) RETURN stddev_samp(p.age) AS s", -1, "", "unknown function `stddev_samp`", nil},
		{"D functions", "percentile-cont", "MATCH (p:Person) RETURN percentile_cont(p.age, 0.5) AS m", -1, "", "unknown function `percentile_cont`", nil},
		{"D functions", "char-length", "RETURN char_length('abc') AS n", -1, "", "unknown function `char_length`", nil},
		{"D functions", "size-string", "RETURN size('abc') AS n", 1, "[3]", "", nil},
		{"D functions", "cardinality", "RETURN cardinality([1,2]) AS n", -1, "", "unknown function `cardinality`", nil},
		{"D functions", "upper-lower", "RETURN upper('ab') + lower('CD') AS x", 1, "[ABcd]", "", nil},
		{"D functions", "trim", "RETURN trim('  x  ') AS x", -1, "", "unknown function `trim`", nil},
		{"D functions", "ltrim-rtrim", "RETURN ltrim('  x') + rtrim('y  ') AS x", -1, "", "unknown function `ltrim`", nil},
		...
		{"D functions", "abs-mod", "RETURN abs(-3) AS a, mod(7, 2) AS m", -1, "", "unknown function `mod`", nil},
		{"D functions", "power-fn", "RETURN power(2, 10) AS x", -1, "", "unknown function `power`", nil},
		{"D functions", "exp-ln-log", "RETURN exp(0) AS e, ln(1) AS l, log10(100) AS g", -1, "", "unknown function `exp`", nil},
		{"D functions", "trig", "RETURN sin(0) AS s, cos(0) AS c, tan(0) AS t", -1, "", "unknown function `sin`", nil},
		...
		{"D functions", "element-id", "MATCH (p {name:'Alice'}) RETURN element_id(p) AS x", -1, "", "unknown function `element_id`", nil},
		{"D functions", "id-fn", "MATCH (p {name:'Alice'}) RETURN id(p) AS x", 1, "", "", nil},
		{"D functions", "labels-fn", "MATCH (p {name:'Alice'}) RETURN labels(p) AS l", -1, "", "unknown function `labels`", nil},
		...
		{"D functions", "head-last-tail", "RETURN head([1,2,3]) AS h", -1, "", "unknown function `head`", nil},
		...
		{"F robustness", "line-comment", "RETURN 1 AS x // trailing comment", -1, "", "unexpected trailing input \"/\"", nil},
		{"F robustness", "block-comment", "RETURN /* mid */ 1 AS x", -1, "", "expected an expression, found \"/\"", nil},
		...
		{"F robustness", "unicode-identifier", "MATCH (héllo:Person) RETURN count(*) AS n", -1, "", "unexpected character \"Ã\"", nil},
		{"F robustness", "quoted-identifier", "MATCH (p:`Weird Label`) RETURN count(*) AS n", -1, "", "unexpected character \"`\"", nil},
