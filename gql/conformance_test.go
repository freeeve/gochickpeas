...
func conformanceCorpus() []conformanceProbe {
	return []conformanceProbe{
		...
		{"A composition", "union-distinct-kw", "MATCH (p:Person) RETURN p.name UNION DISTINCT MATCH (c:City) RETURN c.name", -1, "", "must return the same columns", nil},
		{"A composition", "except", "MATCH (p:Person) RETURN p.name EXCEPT MATCH (c:City) RETURN c.name", -1, "", "must return the same columns", nil},
		{"A composition", "intersect", "MATCH (p:Person) RETURN p.name INTERSECT MATCH (c:City) RETURN c.name", -1, "", "must return the same columns", nil},
		...
		{"A composition", "call-scope-multi", "MATCH (p:Person)-[:KNOWS]->(q:Person) CALL (p, q) { RETURN p.age + q.age AS s } RETURN max(s) AS m", 1, "[65]", "", nil},
		{"A composition", "call-scope-union", "MATCH (p:Person {name: 'Alice'}) CALL (p) { MATCH (p)-[:KNOWS]->(f) RETURN f.name AS x UNION ALL MATCH (p)-[:LIVES_IN]->(c) RETURN c.name AS x } RETURN count(*) AS n", 1, "[3]", "", nil},
		{"A composition", "call-scope-empty", "MATCH (p:Person) CALL () { MATCH (c:City) RETURN c.name AS cn } RETURN count(*) AS n", 1, "[3]", "", nil},
		{"A composition", "call-scope-empty-unbound", "MATCH (p:Person) CALL () { RETURN p.age AS a } RETURN a", -1, "", "unknown variable `p`", nil},
		...
		{"C expressions", "concat-pipes", "RETURN 'a' || 'b' AS x", 1, "[ab]", "", nil},
		...
		{"C expressions", "xor", "RETURN true XOR false AS x", 1, "[true]", "", nil},
		...
		{"C expressions", "is-true", "RETURN (1 < 2) IS TRUE AS x", 1, "[true]", "", nil},
		{"C expressions", "is-unknown", "RETURN (null = 1) IS UNKNOWN AS x", 1, "[true]", "", nil},
		...
		{"C expressions", "value-subquery", "RETURN VALUE { MATCH (c:City) RETURN c.name } AS x", -1, "", "VALUE { ... } scalar subqueries are not supported", nil},
		...
		{"C expressions", "is-typed", "MATCH (x) RETURN x IS TYPED INTEGER AS t LIMIT 1", 1, "[false]", "", nil},
		...
		{"D functions", "percentile-cont", "MATCH (p:Person) RETURN percentile_cont(p.age, 0.5) AS m", 1, "[30]", "", nil},
		{"D functions", "percentile-disc", "MATCH (p:Person) RETURN percentile_disc(p.age, 0.5) AS m", 1, "[30]", "", nil},
		{"D functions", "percentile-nonconst", "MATCH (p:Person) RETURN percentile_cont(p.age, p.age) AS m", -1, "", "requires a constant percentile", nil},
		...
		{"D functions", "normalize", "RETURN normalize('abc') AS x", 1, "[abc]", "", nil},
		{"D functions", "normalize-form-keyword", "RETURN normalize('abc', NFKC) AS x", 1, "[abc]", "", nil},
		{"D predicates", "is-normalized", "RETURN 'abc' IS NORMALIZED AS x, 'abc' IS NOT NFD NORMALIZED AS y", 1, "[true false]", "", nil},
